...
def test(k, model_str, use_orderflow, console):
    ...
    for symbol in symbols:
        ...
        cm = confusion_matrix(y_test_all, y_test_pred_all)
        # Save confusion matrix
        os.makedirs(
            f"test/k={k}/{model_str}/{'OF' if use_orderflow else 'LOB'}/", exist_ok=True
        )

        console.rule(
            f"Confusion matrix for {symbol} with k={k}, {model_str}, {'OF' if use_orderflow else 'LOB'}"
        )
        with open(
            f"test/k={k}/{model_str}/{'OF' if use_orderflow else 'LOB'}/{symbol}_cm.pkl",
            "wb",
        ) as f:
            pickle.dump(cm, f)
        console.rule()
